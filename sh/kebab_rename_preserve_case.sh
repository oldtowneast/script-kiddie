...
# Function to convert string to kebab-case while preserving case
to_kebab_case() {
    echo "$1" | 
...
rename_items() {
    ...
    find "$dir" -depth | while read -r item; do
        ...
        # Convert to kebab-case preserving original case
...
echo "Converting all names to kebab-case while preserving original case"
echo
